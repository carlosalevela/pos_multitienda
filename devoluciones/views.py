...
class CrearDevolucionView(APIView):
    ...
    def post(self, request):
        ...
        if (request.user.rol in ("supervisor", "cajero") and
                _es_otra_tienda(request.user, venta.tienda_id)):
            return Response(
                {"error": "No tienes permiso para devolver "
                          "ventas de otra tienda."},
        ...
        vendido = {d.producto_id: d.cantidad
        ...
        for item in request.data.get("detalles", []):
            ...
            if prod_id not in vendido:
                return Response(
                    {"error": f"Producto ID {prod_id} no pertenece "
                              f"a esta venta."},
                    status=status.HTTP_400_BAD_REQUEST)

            disponible = vendido[prod_id] - ya_devuelto.get(
                prod_id, Decimal("0"))
            if cantidad > disponible:
                return Response(
                    {"error": f"Producto ID {prod_id}: solo quedan "
                              f"{disponible} unidades disponibles."},
                    status=status.HTTP_400_BAD_REQUEST)
...
class CambioProductoView(APIView):
    ...
    def post(self, request):
        ...
        if (request.user.rol in ("supervisor", "cajero") and
                _es_otra_tienda(request.user, venta.tienda_id)):
            return Response(
                {"error": "No tienes permiso para cambiar productos "
                          "de otra tienda."},
                status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            producto_reemplazo_id = int(
                request.data.get("producto_reemplazo"))
            cantidad_reemplazo = Decimal(
                str(request.data.get("cantidad_reemplazo", 0)))
        except (TypeError, ValueError):
            return Response(
        ...
        if cantidad_reemplazo <= 0:
            return Response(
                {"error": "La cantidad del producto de reemplazo "
                          "debe ser mayor a 0."},
                status=status.HTTP_400_BAD_REQUEST)

        try:
            filtro_prod = {"pk": producto_reemplazo_id, "activo": True}
            if not es_superadmin(request):
                filtro_prod["empresa"] = get_empresa(request)
            producto_reemplazo = Producto.objects.get(**filtro_prod)
        except Producto.DoesNotExist:
            return Response(
                {"error": "Producto de reemplazo no encontrado."},
                status=status.HTTP_404_NOT_FOUND)

        inventario_reemplazo = Inventario.objects.select_for_update().filter(
        ...
        if not inventario_reemplazo:
            return Response(
                {"error": "El producto de reemplazo no tiene inventario "
                          "en esta tienda."},
                status=status.HTTP_400_BAD_REQUEST)

        if inventario_reemplazo.stock_actual < cantidad_reemplazo:
            return Response(
                {"error": f"Stock insuficiente para "
                          f"'{producto_reemplazo.nombre}'. "
        ...
        if monto_recibido in ("", None):
            ...
            try:
                ...
                return Response(
                    {"error": "El monto recibido es inválido."},
                    status=status.HTTP_400_BAD_REQUEST)
        ...
        if tipo_diferencia == "cobrar":
            if not metodo_pago_diferencia:
                return Response(
                    {"error": "Debes indicar el método de pago "
                              "de la diferencia."},
                    status=status.HTTP_400_BAD_REQUEST)
            if monto_recibido is None:
                return Response(
                    {"error": "Debes indicar el monto recibido "
                              "para la diferencia."},
                    status=status.HTTP_400_BAD_REQUEST)
            if monto_recibido < diferencia:
                return Response(
                    {"error": f"El monto recibido no cubre la "
        ...
        serializer = DevolucionSerializer(
            data={
                "venta":             venta.id,
                "metodo_devolucion": request.data.get(
...
class CancelarDevolucionView(APIView):
    ...
    def post(self, request, pk):
        try:
            qs = Devolucion.objects.select_related("venta").prefetch_related(
                "detalles__producto")
            devolucion = qs.get(pk=pk) if es_superadmin(request) \
                else qs.get(pk=pk,
                            tienda__empresa=get_empresa(request))
        ...
        if (request.user.rol == "supervisor" and
                _es_otra_tienda(request.user, devolucion.tienda_id)):
            return Response(
                {"error": "No tienes permiso para cancelar devoluciones "
                          "de otra tienda."},
                status=status.HTTP_403_FORBIDDEN)
